...
DB_tobe_searched_against = 'Ecoli_B_BL21_DE3'
Query_species = 'Ecoli_MG1655'
Sbjct_species = 'Ecoli_B_BL21_DE3'


title_PTMsites = "Ecoli_B_BL21_DE3 PTM sites (PAM30)"
title_Proteins = "Ecoli_B_BL21_DE3 Genes with PTMs (PAM30)"
...
Filename_xml_query = "uniprot-proteome_UP000000625_Ecoli_k12_MG1655.xml"
#Accession number of each protein in xml(query)---Acc_csv(query)
Filename_Accession_Number_csv_query = "Accession_Number_Ecoli_k12_MG1655.csv"
#PTM information of xml(query)---PTM_csv(query)
Filename_PTM_info_csv_query = "PTM_information_Ecoli_k12_MG1655.csv"

PathUniprotXML_query = os.path.join(Current_path, Filename_xml_query)
PathUniprotAccessionNumber_query = os.path.join(Current_path, Filename_Accession_Number_csv_query)
PathPTM_query = os.path.join(Current_path, Filename_PTM_info_csv_query)



#Short sequences that generated based on PTM_csv(query)---short_sequence_fasta(query)
Filename_short_sequence_fasta = "short_sequence_tobe_searched_Ecoli_k12_MG1655.csv.fasta"
PathShortFasta = os.path.join(Current_path, Filename_short_sequence_fasta)


# Not in use currently because the parameters in BLAST search are fixed
# Filename_parameters = "Parameters.txt"  # potentially, it should contain the name of score matrix, gap penalty, etc

# The directory that results will be stored in. The folder has to be made prior to code compiling
...
Filename_PTM_info_csv_sbjct = "PTM_information_Ecoli_B_BL21_DE3.csv"
Filename_PTM_merge = "PAM30_Ecoli_MG1655_vs_Ecoli_B_BL21_DE3_intermediate_summary.csv"

PathPTM_sbjct = os.path.join(Current_path, Filename_PTM_info_csv_sbjct)
PathPTM_merge = os.path.join(Current_path, Filename_PTM_merge)


FeatureType=['modified residue', 'lipidation', 'glycosylation site']
Filename_xml_sbjct = "uniprot-proteome_UP000002032_Ecoli_B_BL21_DE3.xml"
Filename_intermediate_csv = "PAM30_Ecoli_MG1655_vs_Ecoli_B_BL21_DE3_intermediate_summary.csv"
Filename_Accession_Number_csv_sbjct = "Accession_Number_Ecoli_B_BL21_DE3.csv"


PathUniprotXML_sbjct = os.path.join(Current_path, Filename_xml_sbjct)
PathUniprotIntermediateCSV = os.path.join(Current_path, Filename_intermediate_csv)
...
merge_Acc_Position_key, sbjct_Acc_Position_key = PTMp.AccNO_PTM_position_extraction(PathPTM_merge, PathPTM_sbjct)
...
intermediate_csv_file = pd.read_csv(PathUniprotIntermediateCSV, dtype={'Query_PTM_position': int, 'Query_start': int, 'Query_end': int, 'Sbjct_PTM_position': int})
...
with open('uniprot-proteome_UP000002032_Ecoli_B_BL21_DE3_PredicedPTM_Added.xml', 'a') as f:
